...
import org.usfirst.frc.team2077.commands.AutonomousOperations;
import org.usfirst.frc.team2077.commands.ColorOperations;
import org.usfirst.frc.team2077.commands.ElevatorOperations;
...
import org.usfirst.frc.team2077.commands.ZPlaceHolderSensors;
import org.usfirst.frc.team2077.commands.EmptyLoader;
import org.usfirst.frc.team2077.commands.Launch;
...
import org.usfirst.frc.team2077.subsystems.ColorWheel;
...
public class Robot extends TimedRobot {
  ...
  public ColorWheel colorWheel_;
  
  //public TestLauncher tLauncher_; // Bringing back support for the TestLauncher Class though the old instance name
  public ColorOperations colorOperations_;
  // public Talon colorTalonMotor_; // use colorWheel_.talon_
  // public Shuffleboard Shuffleboard_;
  ...
  public void robotInit() {
    ...
    // Drivetrain.
    /*
    DriveModule[] dummyDifferentialDriveModules = {
      new DummyDriveModule(robot_.constants_.TESTBOT_TOP_SPEED), // east (right)
      new DummyDriveModule(robot_.constants_.TESTBOT_TOP_SPEED) // west (left)
    };
    */
    /*
    ...
    */
    DriveModuleIF[] starDestroyerDriveModules = {
      new SparkNeoDriveModule(2, true),  // northeast (right front)
      new SparkNeoDriveModule(3, true),  // southeast (right rear)
      new SparkNeoDriveModule(4, false),  // southwest (left rear)
      new SparkNeoDriveModule(1, false)   // northwest (left front)
    };
    ...
    // tLauncher_ = new TestLauncher(); // use testLauncher_ for testing, launcher_ for competition code
    // Shuffleboard_ = new Shuffleboard();
    colorOperations_ = new ColorOperations();
    //colorTalonMotor_ = new Talon(9); use colorWheel_.talon_
    ...
    crosshairs_ = new Crosshairs("Camera0", constants_.FISHEYE_CAMERA_PIXEL_WIDTH, constants_.FISHEYE_CAMERA_PIXEL_HEIGHT, constants_.FISHEYE_CAMERA_FOCAL_LENGTH,
      constants_.FISHEYE_CAMERA_POSITION_NORTH, constants_.FISHEYE_CAMERA_HEIGHT, constants_.FISHEYE_CAMERA_TILT, constants_.UPPER_TARGET_HEIGHT); 
    ...
    driveStation_.secondary10_.whileHeld(new LauncherScrewTest(true));
    driveStation_.secondary11_.whileHeld(new LauncherScrewTest(false));

    // driveStation_.secondary10_.whileHeld(new LauncherScrewTest(new Launcher().setScrewPosition(3)));//launcher_.getScrewPosition() - 0.1)));
    ...
    driveStation_.primaryTrigger_.whileHeld(new RunGrabber(0.3));//RunGrabber: Presently does not do anything, supplementing with runGrabber2, runGrabber appears to be a test class which does not do anything at the moment.


    driveStation_.secondary24_.whenPressed(new ElevatorOperations(true));
/* MAP

  secondary2 = SteerToCrosshairs (continueious)
  secondary10 = moves screw false (continueious)
  secondary11 = moves screw true (continueious)
